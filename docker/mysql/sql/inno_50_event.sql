...
CREATE TABLE `innoEvent` (
  ...
  `EventSendMailOrderSubjectLabel`		varchar(100) NULL DEFAULT '§§MAIL_ORDER_SUBJECT' COMMENT 'token for order email template subject',
  `EventSendMailOrderContentLabel`		varchar(100) NULL DEFAULT '§§MAIL_ORDER_CONTENT' COMMENT 'token for order email template content (text)',
  `EventSaleStartDateBeforeLabel`		varchar(100) NULL DEFAULT '§§SALE_BEFORE_START' COMMENT 'token for text before event sale start date is reached',
  `EventOfflineLabel`					varchar(100) NULL DEFAULT '§§EVENT_OFFLINE' COMMENT 'token for text if the event is offline',
  `EventSaleEndDateAfterLabel`			varchar(100) NULL DEFAULT '§§SALE_AFTER_END' COMMENT 'token for text after event sale end date is reached',
