...
CREATE TABLE `innoTicket` (
  ...
  `TicketLabel`                                  varchar(100) NULL DEFAULT '§§TICKET_LABEL' COMMENT 'label, translation token',
  `TicketDescriptionLabel`                       varchar(100) NULL DEFAULT '§§TICKET_DESCRIPTION_LABEL' COMMENT 'label for detail description text for this ticket, translation token',
...
CREATE TABLE `innoTicketPreprint` (
  `TicketPreprintScanCode`                                        varchar(15) NOT NULL COMMENT 'unique scancode for preprint ticket => 5 chars event prefix, EAN (1 digit fix 0 => 0 is reserved for preprint tickets, 6 digits number, 1 check digit)',
