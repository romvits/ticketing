...
CREATE TABLE `innoOrderDetail` (
  ...
  `OrderDetailType`                  enum('ti','se','sp','sc','hf') NOT NULL COMMENT 'type of order detail => ti=entry ticket | se=seat at location | sp=upselling like Tortengarantie | sc=shipping cost | hf=handling fee',
  `OrderDetailState`                 enum('so','ca') NOT NULL COMMENT 'state of order detail => so=sold | ca=canceled',
